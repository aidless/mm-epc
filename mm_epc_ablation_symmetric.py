"""
MM-EPC Ablation: Symmetric Learning Rates
==========================================
α_win = α_lose = 0.06 (symmetric), removing the asymmetry confound.
...
def train_phase_sym(name, task_list, initial_weights, rounds, seed):
    """Symmetric LR: α_win = α_lose = 0.06"""
    random.seed(seed)
    ...
    for i in range(rounds):
        ...
        for attempt in range(3):
            try:
                ...
                if wn == "A": wins += 1
                upd = 0.06 if wn == "A" else -0.06  # SYMMETRIC
...
CP=os.path.join(OUT,"ablation_symmetric_checkpoint.json")

results=[]; start=1
if os.path.exists(CP):
    with open(CP) as f: ck=json.load(f); results=ck.get("results",[]); start=len(results)+1

print(f"\nSymmetric LR ablation: N={N}, R={R}")
t0=time.time()
...
for rep in range(start,N+1):
    ...
    try:
        w_T=train_phase_sym("T",TEXT_TASKS,init,R,sb+1)
        w_V=train_phase_sym("V",VISUAL_TASKS,init,R,sb+2)
        w_TV=train_phase_sym("TV",VISUAL_TASKS,w_T,R,sb+3)
        w_VT=train_phase_sym("VT",TEXT_TASKS,w_V,R,sb+4)
...
print(f"\nSymmetric: gTV={m_tv:.4f} gVT={m_vt:.4f} diff={m_d:.4f}")
