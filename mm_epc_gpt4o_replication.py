"""
MM-EPC GPT-4o Replication (N=8, 30 rounds)
===========================================
Closes the biggest gap: N=1 GPT-4o result from Phase 2.
Executor: DeepSeek-chat | Evaluator: GPT-4o (via API2D)
8 reps × 4 phases × 30 rounds = 960 TTRL rounds (~2,880 API calls)
"""
...
def train_phase(name, task_list, initial_weights, rounds, seed):
    ...
    pci = (sum((v-1/len(STRATEGIES))**2 for v in w.values())/len(STRATEGIES))**0.5 / (1/len(STRATEGIES))
    top_s = max(w, key=w.get)
    print(f"  [{name}] top={top_s}({w[top_s]:.0%}) wins={wins}/{rounds} PCI={pci:.3f}", flush=True)
    return w

def contagion(w_pure, w_contaminated):
    pv, cv = list(w_pure.values()), list(w_contaminated.values())
...
if __name__ == "__main__":
    ...
    CP = os.path.join(OUTPUT_DIR, "gpt4o_replication_checkpoint.json")
    OUT = os.path.join(OUTPUT_DIR, "mm_epc_gpt4o_replication.json")
    ...
    print(f"\nGPT-4o Replication: N={N}, R={ROUNDS}\n{'='*50}")
    t0 = time.time()
    ...
    for rep in range(start, N+1):
        ...
        try:
            w_T = train_phase("T", TEXT_TASKS, init, ROUNDS, sb+1)
            w_V = train_phase("V", VISUAL_TASKS, init, ROUNDS, sb+2)
            w_TV = train_phase("TV", VISUAL_TASKS, w_T, ROUNDS, sb+3)
            w_VT = train_phase("VT", TEXT_TASKS, w_V, ROUNDS, sb+4)
            ...
            r = {"rep":rep,"gTV":gTV,"gVT":gVT,"asym":asym,"dir":direction,
                 ...
                 "time":time.time()-t_rep}
            results.append(r)
            print(f"  gTV={gTV:.3f} gVT={gVT:.3f} asym={asym:.3f} dir={direction} [{r['time']:.0f}s]", flush=True)
    ...
    for _ in range(10000):
        s = [diffs[random.randint(0,N-1)] for _ in range(N)]
        bs.append(sum(s)/N)
    p2 = sum(1 for d in bs if abs(d)>=abs(m_d))/10000
    pooled = ((sum((gtv[i]-m_tv)**2 for i in range(N))+sum((gvt[i]-m_vt)**2 for i in range(N)))/(2*N-2))**0.5
    d_cohen = abs(m_d)/max(0.001,pooled)
    v2t = sum(1 for r in results if r["gVT"]>r["gTV"])
    t2v = sum(1 for r in results if r["gTV"]>r["gVT"])
    zeros = sum(1 for r in results if r["gTV"]==0 and r["gVT"]==0)

    final = {"experiment":"GPT-4o Replication","N":N,"R":ROUNDS,
             "gTV_mean":round(m_tv,4),"gVT_mean":round(m_vt,4),"diff_mean":round(m_d,4),
             "se_diff":round(se_d,4),"ci95":[round(ci_lo,4),round(ci_hi,4)],
             "p_bootstrap":round(p2,4),"cohens_d":round(d_cohen,4),
             "sig_05":p2<0.05,"dir_VtoT":v2t,"dir_TtoV":t2v,"zeros":zeros,
             "results":results,"total_time":time.time()-t0}
    with open(OUT,"w") as f: json.dump(final,f,indent=2,ensure_ascii=False)

    print(f"""
{'='*50}
GPT-4o N={N}: g_TV={m_tv:.3f} g_VT={m_vt:.3f} diff={m_d:.3f}+/-{se_d:.3f}
95% CI: [{ci_lo:.3f},{ci_hi:.3f}] p={p2:.3f} d={d_cohen:.2f}
Dir: V->T={v2t} T->V={t2v} zeros={zeros}
