"""
MM-EPC Qwen3.7-plus Replication (FREE via Alibaba Cloud)
=========================================================
Replaces the old Qwen-plus (Bailian, N=5, interrupted) with
qwen3.7-plus evaluator + DeepSeek executor. N=8, 30 rounds.
Uses Alibaba free credits (1M tokens). Saves full weights.
"""
import json, os, re, random, time, sys, urllib.request
from copy import deepcopy

DS_KEY = ""
QWEN_KEY = "sk-ab5c91f75f414e57ad5c04089a3ee0df"
...
def qwen_eval(task, ans_a, ans_b, strat_a):
    up = f"Evaluate. Task: {task}\nA ({strat_a}): {ans_a[:300]}\nB (step_by_step): {ans_b[:300]}\nBetter? Output only A or B."
    body = json.dumps({"model":"qwen3.7-plus","messages":[{"role":"user","content":up}],"max_tokens":5,"temperature":0}).encode()
    req = urllib.request.Request("https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions", data=body, method="POST")
    req.add_header("Content-Type","application/json"); req.add_header("Authorization", f"Bearer {QWEN_KEY}")
    ...
    for i in range(rounds):
        ...
        for attempt in range(3):
            try:
                ...
                time.sleep(0.5)  # Alibaba rate limit
                break
            except Exception as e:
                if attempt < 2: time.sleep(3**attempt)
...
print(f"\nQwen3.7-plus N={N}: gTV={m_tv:.3f} gVT={m_vt:.3f} diff={m_d:.3f}+/-{se_d:.3f}")
print(f"95%CI:[{ci_lo:.3f},{ci_hi:.3f}] p={p2:.4f}")
with open(os.path.join(OUT,"mm_epc_qwen37_final.json"),"w") as f: json.dump({"N":N,"gTV":m_tv,"gVT":m_vt,"diff":m_d,"ci95":[ci_lo,ci_hi],"p":p2,"results":results},f,indent=2)
